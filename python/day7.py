...
for j in range(0,100000):
    ...
    if valid(s,[0,1,2,3,4]):
        ...
        q[0].put(0)
        for i in range (4): run(nums.copy(),q[i],q[i+1],[0].copy())
        run(nums.copy(),q[4],q[0],[0])
        res=(q[0].get())
        if res>highestA: highestA=res
    elif valid(s,[5,6,7,8,9]):
        q,c,p,res=[],[],[],0
        for i in range (5): 
            q.append(queue.Queue(maxsize=20))
            c.append(nums.copy())
            p.append([0])
        for i in range (5): q[i].put(int(s[i]))
        q[0].put(0)
        while res !=-99999:
            for i in range (4): run(c[i],q[i],q[i+1],p[i])
            res = run(c[4],q[4],q[0],p[4])
        res=q[0].get()
        if res>highestB: highestB=res
print("AnswerA: ",highestA)
print("AnswerB: ",highestB)